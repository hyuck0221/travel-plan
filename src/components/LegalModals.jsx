    ...
    <Modal title="개인정보처리방침" isOpen={isOpen} onClose={onClose}>
        ...
        <div className="legal-section">
            ...
            <p>사용자가 입력한 여행 일정, 장소 메모 등은 사용자의 브라우저 내에서 인코딩되어 URL 형태로 생성됩니다. 이 데이터는 본 서비스의 서버를 거치지 않으며, 오직 링크를 가진 사람만이 확인할
                수 있습니다.
                <br></br>
                일정 데이터는 브라우저의 로컬 스토리지에 저장되어 재접속 시 활용될 수 있습니다. AI 대화 이력은 새로고침 전까지 브라우저 메모리에만 유지되며 별도로 저장하지 않습니다.
                <br></br>
        ...
        <div className="legal-section">
            ...
            <p>Travelink AI 기능은 WebGPU를 이용해 기본적으로 Qwen 3 1.7B 초경량 로컬 모델을 사용자의 브라우저에서 실행하며, 더 큰 모델을 선택했을 때 기기의 WebGPU 메모리가 부족하면 이 모델로 전환할 수 있습니다. 사용자가 입력한 AI 프롬프트, 현재 일정, AI 대화 이력은 일정 작업을 해석하고 반영하기 위해 브라우저 내에서 처리되며, 생성형 AI 모델의 입력·출력 내용이 Travelink 서버나 별도의 외부 생성형 AI 서비스로 전송되거나 저장되지 않습니다.
                <br></br>
                로컬 모델 파일은 AI 실행을 위해 브라우저 저장소에 캐시될 수 있으며, 이는 사용자의 프롬프트나 일정 데이터와 별개입니다. AI 모델 설정은 다음 방문에도 선택을 유지하기 위해 브라우저 로컬 스토리지에 저장됩니다.
                <br></br>
                ...
                <br></br>
                AI 설정에서 외부 API를 선택하면 사용자의 프롬프트, 현재 일정, 대화 이력이 선택한 AI 제공자의 API로 전송될 수 있으며 해당 제공자의 정책이 적용됩니다. 입력한 API key와 외부 AI 설정은 다음 방문에도 연결을 유지하기 위해 브라우저 로컬 스토리지에 저장됩니다.
                <br></br>
